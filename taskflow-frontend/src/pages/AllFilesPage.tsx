...
export default function AllFilesPage() {
  ...
  const { data: files = [] } = useQuery({ queryKey: ["all-files"], queryFn: () => filesService.allFiles() });
  ...
  return (
    <div className="p-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-[#1f2937] p-6">
        <h3 className="mb-4 text-2xl font-bold">All Files ({files.length})</h3>
        ...
        <div className="divide-y divide-[#374151]/50">
          ...
          {files.length === 0 && <p className="py-8 text-center text-sm text-white/40">No files yet</p>}
        </div>
      </motion.div>
    </div>
  );
